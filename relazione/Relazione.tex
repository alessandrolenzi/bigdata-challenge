...
In our initial approach, a mere visit, ordered on the arc identifier (i.e. \texttt{for i = 0 to 10000 do strongconnect(i)})
has been performed. This visit, though performed with different cuts over the probability of the arcs, evidenced
...
\item and second, the "static" threshold did not adapted well to changing traffic along the days.
\end{itemize}
...
To do so, we choosed the \emph{Markov Clustering} algorithm, which is known in litterature for the purpose of finding communities in graphs.


For a more complete dissertation on what we did using this approach for discovering connected components, we invite you to refer to the attacched file \texttt{CfcSuGrafiOrari.pdf}, in Italian.
